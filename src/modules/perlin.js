import {
  Vector, dotProduct, vectorFromIndex, bilinearInterpolation,
} from './vectors';
...
perlin noise: random noise where each point is related to the points around it
  three steps:
  1. create grid
    the grid has to be fewer points than the end result array
    (a 256x256 end result would want something between 8x8 and 64x64)
    this grid needs random normalized vectors on each node
  2. dot products for each point
    each point in the end result must have dot product for the surrounding 4 and
    the directional vector from large grid node to specific end result square
    (in 2d space) calculated (four dot products):
      dot product of two vectors = v1.x * v2.x + v1.y * v2.y
  3. bilinear interpolation of the four dot products to the end result point
    (better images with cosine interpolation)
  4. perlin noise actually uses a fade function--after final color is determined
    it is faded by the function: 6t^5 - 15t^4 + 10t^3
*/
...
function perlinNoise(w, h, overlayWidth, overlayHeight) {
  ...
  for (let i = 0; i < perlin.length; i += 1) {
    const v = vectorFromIndex(i, w);

    let noiseValue = perlinNoiseForPoint(
      new Vector(v.x / widthDivisions / 2, v.y / heightDivisions / 2),
      largeUnitGrid,
      overlayWidth / 2,
    );
  ...
  for (let i = 0; i < perlin.length; i += 1) {
    ...
    let noiseValue = perlinNoiseForPoint(
      new Vector(v.x / widthDivisions, v.y / heightDivisions),
      unitGrid,
      overlayWidth,
    );
    noiseValue = noiseValue * 0.5 + 0.5;
